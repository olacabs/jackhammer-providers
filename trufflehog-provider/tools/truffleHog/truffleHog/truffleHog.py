...
def main():
    ...
    parser.add_argument("--outputFile", type=str,dest="outputFile", help="Output File")
    ...
    output = find_strings(args.since_commit, args.max_depth, args.output_json, args.do_regex, do_entropy, surpress_output=False, branch=args.branch, repo_path=args.repo_path)
    ...
    for line in lines:
        for word in line.split():
            ...
            for string in base64_strings:
                ...
                if b64Entropy > 4.5:
                    stringsFound.append(string[0:200])
...
def handle_results(output,foundIssues):
    for foundIssue in foundIssues:
        output["foundIssues"].append(foundIssue)
        #result_path = os.path.join(output_dir, str(uuid.uuid4()))
        #with open('test.json', "w+") as result_file:
            #output["foundIssues"].append()
            #result_file.write(json.dumps(foundIssue))
        #output["foundIssues"].append(result_path)
    return output

def find_strings(since_commit=None, max_depth=1000000, printJson=False, do_regex=False, do_entropy=True, surpress_output=True, custom_regexes={}, branch=None, repo_path=None):
    output = {"foundIssues": []}
    #if repo_path:
    ...
    for remote_branch in branches:
        ...
        for curr_commit in repo.iter_commits(branch_name, max_count=max_depth):
            ...
            output = handle_results(output,foundIssues)
            prev_commit = curr_commit
