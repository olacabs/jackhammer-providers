...
public class PostDeployController implements PostDeploySpi {
	...
	public ProcessBuilder buildCommand() {
		...
		ProcessBuilder builder = new ProcessBuilder(Constants.NOHUP,Constants.SH,env.get(Constants.FIND_SEC_BUGS_POST_DEPLOY_SCRIPT_PATH));
		builder.redirectError(new File(env.get(Constants.FIND_SEC_BUGS_POST_DEPLOY_ERROR_LOG_PATH)));
		builder.redirectOutput(new File(env.get(Constants.FIND_SEC_BUGS_POST_DEPLOY_OUTPUT_LOG_PATH)));
		return builder;
	}
