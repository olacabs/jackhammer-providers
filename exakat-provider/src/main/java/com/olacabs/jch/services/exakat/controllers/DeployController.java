...
public class DeployController implements DeploySpi {
    ...
    public ProcessBuilder buildCommand() {
        ProcessBuilder builder = new ProcessBuilder(Constants.NOHUP,Constants.SH,System.getenv(Constants.EXAKAT_DEPLOY_SCRIPT_PATH));
        builder.redirectError(new File(System.getenv(Constants.EXAKAT_DEPLOY_ERROR_LOG_PATH)));
        builder.redirectOutput(new File(System.getenv(Constants.EXAKAT_DEPLOY_OUTPUT_LOG_PATH)));
        return builder;
    }
