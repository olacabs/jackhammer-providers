...
public class PreDeployController implements PreDeploySpi {
    ...
    public ProcessBuilder buildCommand() {
        ProcessBuilder builder = new ProcessBuilder(Constants.NOHUP, Constants.SH, System.getenv(Constants.EXAKAT_PRE_DEPLOY_SCRIPT_PATH));
        builder.redirectError(new File(System.getenv(Constants.EXAKAT_PRE_DEPLOY_ERROR_LOG_PATH)));
        builder.redirectOutput(new File(System.getenv(Constants.EXAKAT_PRE_DEPLOY_OUTPUT_LOG_PATH)));
        return builder;
    }
