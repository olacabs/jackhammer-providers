...
public class DeployController implements DeploySpi {
    ...
    public ProcessBuilder buildCommand() {
        ProcessBuilder builder = new ProcessBuilder(Constants.NOHUP,Constants.SH,System.getenv(Constants.NMAP_DEPLOY_SCRIPT_PATH));
        builder.redirectError(new File(System.getenv(Constants.NMAP_DEPLOY_ERROR_LOG_PATH)));
        builder.redirectOutput(new File(System.getenv(Constants.NMAP_DEPLOY_OUTPUT_LOG_PATH)));
        return builder;
    }
	...
	public int executeCommand(ProcessBuilder processBuilder) {
        ...
        try {
            ...
            exit = -1;
            log.error("IOException while installing NMAP scanner", e);
